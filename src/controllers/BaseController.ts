...
export abstract class BaseController {
    ...
    protected async handleGetAll(
        ...
        logger.debug(`Fetching all ${resourceName}...`, { correlationId });
        ...
        try {
            ...
            logger.debug(`Successfully fetched ${result.data.length} ${resourceName}`, { correlationId });
            res.json(result);
        } catch (error: any) {
            logger.debug(`Error fetching ${resourceName}: ${error.message}`, { correlationId, error });
